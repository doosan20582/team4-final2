...
public class OrderController {
	...
	public String orderGet(HttpServletRequest request, @RequestParam String product_id, @RequestParam String order_quantity, Model model) {
	...
	public String buy(OrderVO orderVO, HttpServletRequest request, @RequestParam double getPoint, Model model) {
		log.info("상품구매 ==================================================================");
		log.info("주문 정보 : " + orderVO);
		log.info("주문 고객 아이디 : " + request.getSession().getAttribute("login_id").toString());
		log.info("구매 후 적립 포인트 : " + getPoint);
		//구매자 아이디 세션에서 받아오기
		...
		log.info(member_purchase_point);
		...
		if(resultText.equals("상품을 구매하였습니다.")){
			...
			model.addAttribute("msg", resultText);
			model.addAttribute("url", "/order/buy?product_id=" + query1 + "&" +"order_quantity=" + query2);
//			model.addAttribute("url", "/order/buy?product_id=" + orderVO.getProduct_id() + "&order_quantity=" + orderVO.getOrder_quantity());
		}
